...
do {
    ...
    $uri = "https://" + $vraServer + '/deployment/api/deployments?$top=' + $top + '&$skip=' + $found
    $result = Invoke-WebRequest -uri $uri -Method GET -Headers $header 

    if ( $result.StatusCode -eq 200 ) {
        $resultJSON = ( $result | ConvertFrom-Json )
        if ( $resultJSON.numberOfElements -gt 0) {
            $arrDeployments += $resultJSON.Content
        }
    }
    $found += $resultJSON.numberOfElements
}
while ( $resultJSON.totalElements -gt $found )

$arrDeployments.count
